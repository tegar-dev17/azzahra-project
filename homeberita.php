    ...
    <div class="container mx-auto px-4 flex flex-wrap justify-center pt-10">
            ...
            <div class="mb-4 p-4 ">
                ...
                <?php foreach ($asideData as $data) : ?>
                    <div class="mb-4 p-4 flex items-start transition-all duration-300 hover:bg-gray-100">
                        <img src="<?= $data['photo']; ?>" alt="Profile Image" class="w-24 h-24 object-cover shadow-md mr-2 transform transition-transform duration-500 hover:scale-95">
                        <div>
        ...
        <div class="w-full md:w-3/4 p-4">
            <div class="flex flex-wrap justify-center" id="berita">
                <div class="rounded shadow-lg min-w-full max-w-md mx-auto">
                    <div class="w-full h-full mb-20">
                        <?php foreach ($beritaTampil as $berita) : ?>
                            <div class="w-full mt-8 px-4"> <!-- Tambahkan padding ke sisi kiri di sini -->
                                <div class="image-container w-full">
                                    <img src="<?= $berita['photo']; ?>" alt="" class="w-full h-auto max-h-96 max-w-full object-cover" />
                                </div>
                                <br>
                                <i class="fas fa-calendar text-blue-900 md:text-base text-sm"></i>
                                <font class="ml-1 text-gray-600 md:text-base text-sm"><?= date('Y-m-d H:i', strtotime($berita['tanggal'])); ?></font>
                                <i class="fas fa-user md:text-base text-sm ml-2 text-blue-900"></i>
                                <font class="md:text-base text-sm text-gray-600"><?= $berita['admin_username']; ?></font>
                                <h3 class="font-semibold md:text-2xl text-lg md:mt-5 mt-2 md:mb-3 text-black"><?= $berita['judul']; ?></h3>
                                <p class="md:text-lg text-base text-slate-600 font-extralight">
                                    <?= $berita['headline']; ?>
                                </p>
                                <br>
                                <!-- Read More Link -->
                                <a href="#" onclick="tampilkanBerita('<?= $berita['id_berita']; ?>')" class="mt-10 md:text-base text-sm font-semibold text-white bg-blue-900 py-3 md:px-8 px-5 rounded-lg hover:shadow-lg hover:opacity-80 transition duration-300 ease-in-out">Selengkapnya</a>
                            </div>
                        <?php endforeach ?>
